 ...
 */
package com.masary.controlers.LegitimacyAssociation;
 ...
 * @author Abdelsabor
 */
public class LegitimacyAssociationPayController extends HttpServlet {
    ...
    private String DB(HttpServletRequest request, HttpServletResponse response) throws UnsupportedEncodingException {
        ...
        agentPaymentRequestDTO.setNAME(billingAccount);
        ...
        agentPaymentRequestDTO.setOPERATION_ID(Integer.parseInt(request.getParameter(CONFIG.OPERATION_ID)));
        ...
        if (agentPaymentRespponseDto.getSTATUS_CODE().equals("200") || agentPaymentRespponseDto.getSTATUS_CODE().equals("202")) {
            request.setAttribute("agentPaymentRespponseDto", agentPaymentRespponseDto);
            nextJSP = CONFIG.ACTION_LA_PAY;
        } else {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            boolean validateAmount = validateAmount(Double.parseDouble(request.getParameter("donationValue")));
            if (validateAmount == false) {
                session.setAttribute("ErrorMessage", "الحد الادنى للتبرع 10 جنيهات بدون كسور");
                throw new Exception("الحد الادنى للتبرع 10 جنيهات بدون كسور");

            } else {
                String DbNextJSP = DB(request, response);
                if (DbNextJSP.equals(CONFIG.ACTION_LA_PAY)) {
                    nextJSP = CONFIG.LegitimacyAssociationResultPage;
                } else {
                    nextJSP = CONFIG.PAGE_ERRPR;
