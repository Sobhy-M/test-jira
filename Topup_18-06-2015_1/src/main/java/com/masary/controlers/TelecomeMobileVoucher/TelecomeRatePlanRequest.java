 ...
 * @author Ahmed Saeed
 */
public class TelecomeRatePlanRequest extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Double voucherAmountLE = Double.valueOf(request.getParameter("AMOUNT")) / 100;
...
//        MasaryManager.logger.info("******************** " + proxyAuthorizationToken);
        ...
        String ratePlanCommissionsUrlWithAmount = ratePlanCommissionsUrlWithServiceId.replace("{amount}", String.valueOf(voucherAmountLE) + "/");
        session.setAttribute("amount", String.valueOf(voucherAmountLE));
        session.setAttribute("denoId", request.getParameter("DENOMINATION_ID"));
        ...
        try {
            ...
            if (ratePlanCommissionResponse.getStatusLine().getStatusCode() == 200) {
                ...
                String commFees = commissionTransaction.getAppliedFees() + "-"
                        + commissionTransaction.getMerchantCommission() + "-"
                        + "0" + "-"
                        + commissionTransaction.getVatValue() + "-"
                        + (commissionTransaction.getToBePaid() - commissionTransaction.getMerchantCommission()) + "-"
                        + commissionTransaction.getToBePaid();
                out.write(commFees);
