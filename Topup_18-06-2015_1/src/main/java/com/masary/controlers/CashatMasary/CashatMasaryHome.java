...
public class CashatMasaryHome extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			MasaryManager.logger.info("ErrorMessage " + ex.getMessage(),ex);
	...
	private String checkCompaniesAvaliablty(HttpServletRequest request) throws Exception {
		...
		MasaryManager.logger.info("Account "+accountId+" check cashat masary companies avalablity");
		CashatCompniesDTO cashatCompnies = null;

		if (request.getSession().getAttribute("cashatMasaryCompanies") == null) {

			MasaryManager.logger.info("Account "+accountId+" didn't found cashat masary companies and call client to get it");
			...
			if (cashatCompnies == null) {
				MasaryManager.logger.error("ErrorMessage " + "Account "+accountId+" didn't found cashat masary companies");
				session.setAttribute("ErrorMessage", "حدث خطأ أثناء تنفيذ العملية");
				nextPage = CONFIG.PAGE_ERRPR;
			} else {
				request.getSession().setAttribute("cashatMasaryCompanies", cashatCompnies);
			}

		}
		nextPage = CashatMasaryProperties.PAGE_CashatMasary_Home;
