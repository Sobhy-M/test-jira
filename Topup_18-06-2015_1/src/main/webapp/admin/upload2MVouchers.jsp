...
<html>
    <head>
        ...
        <script type="text/javascript">
            ...
            <%--str="<select name='<%=CONFIG.PARAM_CATEGORY_ID%>'>";--%>
            <% for (ProviderDTO provider : providers) {%>

                    if(providerID==<%=provider.getId()%>)   {

            <%--alert(<%=provider.getId()%>);--%>
            <%
            ...
            <%--      {
                  ...
                  <% for (ProviderDTO provider : providers) {%>
                              case <%=provider.getId()%>:
                                      {
                                      document.getElementById("category").innerHTML="<select name='<%=CONFIG.PARAM_CATEGORY_ID%>'>";
                                    
                  <%
                       for (CategoryDTO category : provider.getCategories()) {
                  %>
                                      document.getElementById("category").innerHTML=+ '<option value="<%=category.getId()%>" ><%=category.getName()%></option>'
                  <%
                       }
                  %>
