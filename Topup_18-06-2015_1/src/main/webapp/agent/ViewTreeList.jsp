...
<% int startOfLoop = 0;
    ...
    List<Object[]> customers = MasaryManager.getInstance().getAgentTree((String) request.getSession().getAttribute("PIN"));
    session.setAttribute("treeList", customers);
%>
...
<html>
    <head>
        ...
        <script type="text/javascript">
            ...
            var mydiv = document.getElementById('mydiv');</script>
        ...
        <div id ="menu">
            ...
            <div class="content_body">   
                    ...
                    <tr>
                        <td>
                            <table border="0" >
                                ...
                                <tr>
                                    <th><%=CONFIG.getAccountNumber(session)%></th>
                                    <td>
                                             <input   list="ids" value="<%=requestParameters.get(CONFIG.PARAM_CUSTOMER_ID) != null
                                                ? requestParameters.get(CONFIG.PARAM_CUSTOMER_ID) : ""%>" name="<%=CONFIG.PARAM_CUSTOMER_ID%>" />
                                <datalist id="ids">
                                    <select  size="5">
                                        <%for (Object[] customer : customers) {%>
                                        <option label="<%=customer[1]%>" value="<%=customer[0]%>" />
                                        <%}%>
                                    </select>
                                </datalist>      
                        </td>
                    ...
                    <tr>
                        ...
                        <td>
                            <select name="<%=CONFIG.PARAM_TRANSACTION_TYPE%>">
                                <option value="<%=requestParameters.get(CONFIG.PARAM_TRANSACTION_TYPE)%>" 
                                        <%=requestParameters.get(CONFIG.PARAM_TRANSACTION_TYPE) != null ? "selected" : ""%> ><%=requestParameters.get(CONFIG.PARAM_TRANSACTION_TYPE) == null ? "" : s%></option>
                ...
                <table align="right" style="border-color: #AACCDE; ">
                    <input type="hidden" name="action" value="<%=CONFIG.ACTION_TREE_LIST_TRANS%>" />
                    ...
                    <tr>
                        <td>
                            <table border="1" style="border-color: #AACCDE; " >
                                <tr>
                                    <td colspan="3">
                                        <%=CONFIG.getBalance_FAA(session)%> :
                                    </td>
                                    <td><%= MasaryManager.getInstance().getCustomerInfo((String) requestParameters.get(CONFIG.PARAM_CUSTOMER_ID)).getCurrentBalance()%></td>
                                </tr>
                            </table>
                        </td>
                    </tr>
