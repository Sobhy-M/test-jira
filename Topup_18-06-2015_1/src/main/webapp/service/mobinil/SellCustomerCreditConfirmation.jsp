...
<html>
    <head>
        ...
        <title><%=CONFIG.getConfirm(request.getSession())%> <%=CONFIG.getCustomerMobinilTransfer(request.getSession())%></title>
    ...
    <BODY class="body" onload="ray.hide();">
        ...
        <input type="hidden" name="<%=CONFIG.TOPUP_TYPE%>" value="9" />
        <input type="hidden" name="<%=CONFIG.CONFIRM%>" value="True" />
        <fieldset style="width: 60%; direction: rtl;" align="right">  

            <legend align="right" ><font size="5"><%=CONFIG.getConfirm(request.getSession())%> <%=CONFIG.getCustomerMobinilTransfer(request.getSession())%></font><img src="img/CashIn.ico"  width="20" height="20" > </legend> 
            <table border="1" width="100%">
                ...
                <tr>
                    ...
                    <td><%=CONFIG.getNewBalance(request.getSession())%> : <%=myFormatter.format(mobinilTransferBalance - Double.parseDouble(request.getParameter("DeductedAmount")))%></td></tr>
            </table>
            <details open="open">
                <summary></summary>
