...
<%
    ...
    double mobinilTransferBalance = MasaryManager.getInstance().getCustomerServiceBal(agent.getPin(), 9);
...
<html>
    <head>
        ...
        <title><%=CONFIG.getCustomerMobinilTransfer(request.getSession())%></title>
        <script>
    ...
    <BODY class="body">
        ...
        <form name="BayCreditCustomer" action="<%=CONFIG.APP_ROOT%>Web" method="POST" onsubmit="return validateEtisalatTransfer()">
        ...
        <fieldset style="width: 55%; direction: rtl;" align="right">  
            <legend align="right" ><font size="5"><%=CONFIG.getCustomerMobinilTransfer(request.getSession())%></font><img src="img/CashIn.ico"  width="20" height="20" > </legend> 
            <table border="1" width="100%">
                ...
                <tr>
                    <td > <input type="submit" name="btnSubmit" tabindex="4"  value="<%=CONFIG.getGo(request.getSession())%>" class="Btn" <%=mobinilTransferBalance == 0 ? "disabled='true'" : " "%> ></td>
                    <td><%=CONFIG.getAllowedBalance(request.getSession())%> : <%=agent.isAutoAllocate().equals("F") ? myFormatter.format(customerBalance) : myFormatter.format(mobinilTransferBalance)%></td>
                </tr>
            </table>
            <details open="open">
                <summary></summary>
                ...
                ملحوظة :- </br>
                •المبالغ التى يمكن شحنها فى اورنچ  يجب أن تكون مبالغ صحيحة بدون كسور و هى: من 2 ج إلى 20 جنيه بزيادة 1 جنيه مثل 3 ج و 4 ج و هكذا ثم 25 ج ثم من 30 ج إلى 100 ج بزيادة 10 جنيه مثل 40 ج و 50 ج و هكذا ثم 200 ج و 250 ج و 300 ج و 500 ج.  
