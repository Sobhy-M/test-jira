<%-- 
    Document   : ValuReprint
    Created on : August 28, 2019, 1:53:32 PM
    Author     : Mai Hamdy
--%>
...
%>
<%
    ValuPaymentResponse paymentResponse = (ValuPaymentResponse) request.getSession().getAttribute("valuPaymentResponse");
    double paidAmount = paymentResponse.getPaidAmount();
    String globalTrxID = paymentResponse.getGlobalTrxId();
    String phoneNumber = paymentResponse.getMobilNumber();
    double feesVAT = paymentResponse.getAppliedFeesPlusTax();
...
<title><%=CONFIG.getPrinting_Receipt(request.getSession())%></title>
        <script>
            function setDivPrint() {
                ...
                printwindow.document.write('<tr><td style="padding-left:15px;width: 50%;"><%=feesVAT%></td><td style = "text-align: right;padding-right:25px;width:50%;">:تكلفة الخدمة شامل قيمة الخدمة المضافة </td> </tr><tr>');
    ...
    <body class="body">
        <div>
            ...
            <form name="reprintValuTansaction" action="<%=CONFIG.APP_ROOT + (String) request.getSession().getAttribute(CONFIG.PARAM_ROLE) + ".jsp"%>" method="POST"
            <div>
                <table cellspacing="10" style="font-size: 12px; font-weight: 900;"  > 
                    ...
                    <tr><td style="  width: 50% ; padding-right: 30px;text-align: right;">تاريخ و توقيت العملية</td><td style="padding-left: 10px;"><%=date1%> <%=time%></td></tr>
                    <tr><td style="width: 50% ; padding-right: 30px;text-align: right;"> المبلغ المدفوع </td><td style="padding-left: 10px;"><%= paidAmount%></td></tr>
                    ...
                    <tr><td style="width: 50% ; padding-right: 30px;text-align: right;">عموله التاجر </td><td style="padding-left: 10px;"><%= merchantCommission%></td></tr>
                     <tr><td style="width: 50% ; padding-right: 30px;text-align: right;">تكلفة الخدمة شامل ضريبة القيمة المضافة </td><td style="padding-left: 10px;"><%= feesVAT%></td></tr>
                    <tr><td style="width: 50% ; padding-right: 30px;text-align: right;">إجمالي المبلغ المدفوع </td><td style="padding-left: 10px;"><%= toBePaid%></td></tr>
