<%--
    Document   : AddCustomer.jsp
    Created on : 06/05/2009, 11:09:49 ص
    Author     : Melad
--%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>

<%@page import="com.masary.common.CONFIG"%>
<%@page import="com.masary.controlers.GoBus.GoBusProperties"%>


<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN"
    "http://www.w3.org/TR/html4/loose.dtd">
<%
	String role = (String) request.getSession().getAttribute(CONFIG.PARAM_ROLE);
	if (role == null) {
		response.sendRedirect(CONFIG.APP_ROOT + CONFIG.PAGE_LOGIN);
		return;
	}
%>


<html>


<link
	href="https://cdn.e-masary.net/app/img/style<%=request.getSession().getAttribute(CONFIG.lang)%>.css"
	rel="stylesheet" type="text/css">
<head>
<meta http-equiv="Content-Type"
	content="text/html; charset=windows-1256">
<title><%=GoBusProperties.getGoBus(session)%></title>
<script src="https://code.jquery.com/jquery-1.12.4.js"></script>
</head>

<script type="text/javascript">
	function gotoHome() {
		document.getElementById("homeForm").submit()
	}
</script>

<BODY class="body">
	<div>
		<%
			if (request.getSession().getAttribute(CONFIG.lang).equals("")) {
		%>
		<jsp:include page="../../img/menuList.jsp"></jsp:include>
		<%
			} else {
		%>
		<jsp:include page="../../img/menuListar.jsp"></jsp:include>
		<%
			}
		%>
	</div>

	<form name="AddCustomer" action="GoBusConfirmationController"
		method="POST">

		<div class="content_body">
			<table>
				<div id="errorMessage"></div>
				<tr>
				 <td colspan="2" style="text-align: center;"> <%=GoBusProperties.getRequiredInfo(session)%></td>
				</tr>
				<tr>
					<th><%=GoBusProperties.getReferanceNumber(session)%> *</th>
					<td>
						<div class="requriedclass">
							<div>
								<input type="text" maxlength="20" autocomplete="off"
									name="referanceNum" id="MSISDN" />
							</div>
						</div>
					</td>
				</tr>
				<tr>
					<td style="text-align: center"><input type="submit"
						name="btnSubmit" value="<%=GoBusProperties.getEnquiry(session)%>"
						onclick="return BtnSubmit()"></td>

					<td style="text-align: center"><input type="button"
						name="btnSubmit" value="<%=GoBusProperties.getCancel(session)%>"
						onclick="gotoHome()"></td>

				</tr>
			</table>

		</div>
		<!-- End of Table Content area-->
	</form>

	<form action=<%=CONFIG.APP_ROOT + role + ".jsp"%> method="post"
		name="homeForm" id="homeForm"></form>

	</div>
	<!-- End content body -->
	<div style="clear: both;">&nbsp;</div>
	<div id="footer"><jsp:include page="../../img/timeout.jsp"></jsp:include></div>
	</div>
	<!-- End of Main body-->
</body>
</html>
