...
<html>
    <head>
        ...
        <title><%=CONFIG.getConfirm(request.getSession()) + " " + CONFIG.getCustomerVodafoneTopUp(request.getSession())%></title>
    ...
    <BODY class="body" onload="ray.hide();">
        ...
        <input type="hidden" name="<%=CONFIG.TOPUP_TYPE%>" value="10" />
        ...
        <fieldset style="width: 70%; direction: rtl;" align="right">  
            ...
            <legend align="right" ><font size="5"><%=CONFIG.getConfirm(request.getSession()) + "  " + CONFIG.getCustomerVodafoneTopUp(request.getSession())%></font><img src="https://cdn.e-masary.net/app/img/CashIn.ico"  width="20" height="20" > </legend> 
            <table border="1" style="width: 100%">
                <th><%=CONFIG.getINFO_Required(request.getSession())%></th>
                <th><%=CONFIG.getMerchantCommession(request.getSession())%></th>
                <tr>
                    <td><p align="right"><%=CONFIG.getAmount(request.getSession())%> : <input type="text" name="<%=CONFIG.AMOUNT%>" readonly tabindex="1" id="custTopUpDalanceID" value="<%= new String(requestParameters.get(CONFIG.AMOUNT).getBytes("ISO-8859-1"), "utf-8")%>" style="background-color: #EDEDED; float: left;"/></p> 
                        <p align="right"><%=CONFIG.getMobileNumber(request.getSession())%> :<input id="custMobileConfirmation" type="text" name="<%=CONFIG.PARAM_MSISDN%>" readonly value="<%= requestParameters.get(CONFIG.PARAM_MSISDN)%>" tabindex="2" style="background-color: #EDEDED; float: left;">
                ...
                <tr>
                    <td>
                        <input type="submit" name="btnSubmit" id="btnSubmit" tabindex="3" value="<%=CONFIG.getConfirm(request.getSession())%>" style="float: right" onclick="">
                        <input type="button" name="MoreInfo" tabindex="3" value="<%=CONFIG.getBack(request.getSession())%>"  style="float: left" onclick="history.go(-1);">
                    </td>
                    <td><%=CONFIG.getNewBalance(request.getSession())%> : <%=myFormatter.format(vodafonBalance - Double.parseDouble(requestParameters.get("DeductedAmount")))%></td></tr>
            </table>
            <details open="open">
                <summary></summary>
