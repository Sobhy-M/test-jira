...
<html>
    <head>
        ...
        <title><%=CONFIG.getCustomerEtisalatTransfer(request.getSession())%></title>
        <script>
    ...
    <BODY class="body">
        ...
        <fieldset style="width: 55%; direction: rtl;" align="right">  
            <legend align="right" ><font size="5"><%=CONFIG.getCustomerEtisalatTransfer(request.getSession())%></font><img src="img/CashIn.ico"  width="20" height="20"> </legend> 
            <table border="1" width="100%">
                ...
                <tr>
                    <td > <input type="submit" name="btnSubmit" tabindex="4"  value="<%=CONFIG.getGo(request.getSession())%>" class="Btn" <%=etisalatTransferBalance == 0 ? "disabled='true'" : " "%> ></td>
                    <td><%=CONFIG.getAllowedBalance(request.getSession())%> : <%=agent.isAutoAllocate().equals("F") ? myFormatter.format(customerBalance) : myFormatter.format(etisalatTransferBalance)%></td>
                </tr>
            </table>
            <details open="open">
                <summary></summary>
