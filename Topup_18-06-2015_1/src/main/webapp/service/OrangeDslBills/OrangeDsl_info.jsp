    ...
    <form name="dobillinquiry" action="<%=CONFIG.APP_ROOT%>Bill_Controler" method="POST" onsubmit="return validateBillBayCredit(<%=BTC.isIS_FRAC_ACC()%>)">
        ...
        <div class="content_body"  >
            <fieldset style="width: 70%; direction: rtl;" align="right">  
                <legend align="right" ><font size="5"><%= CONFIG.getCustomerBillHead(session)%>&nbsp;<%= BTC.getStrBTCName(session)%></font><img src="https://cdn.e-masary.net/app/img/CashIn.ico"  width="20" height="20" > </legend>               
                <table  border="1" width="100%">
                    ...
                    <tr>
                        <td>
                            ...
                            <p align="right"><%=CONFIG.GetMinPaymentAmount(session)%> : 
                                <input type="text" name="MinPaymentAmount" tabindex="1"  id="MinPaymentAmount" value="${bill_Response.minimumPayment}" required  readonly style=' background-color: #EDEDED;  float: left;' >
                            </p>

                            
                            <p align="right"><%=CONFIG.getAssign_v(session)%> : 
                                <input type="text" name="<%=CONFIG.AMOUNT%>" tabindex="1"  id="custAmountID" pattern="^[1-9]\d*(\.\d{1,9})?" value="${bill_Response.AMOUNT}" required  onchange="onValueChanged();" style=' float: left;' >
                            </p>
