...
<%@page import="com.masary.integration.dto.OneCardDenominationResponse"%>
<%@page import="com.masary.integration.OneCardClient"%>
...
<%
    ...
    List<OneCardDenominationResponse> oneCardList = (List<OneCardDenominationResponse>)request.getSession().getAttribute("vouchersList");
    ...
    <head>
        ...
        <script language='javascript' type='text/javascript'>
            ...
            function changeValue(){
                var options = document.getElementById('myselection').options ;
                var id = options[options.selectedIndex].value;
                document.getElementById('donationValue').value = id;
            }
        </script>
       ...
       <body onload="onloadFunction()">
        ...
        <form action="OneCardController" id="DonationForm" method="POST" style="font-weight: bold">
             <input type="hidden" name="action" value="<%=CONFIG.ACTION_ONE_CARD_CONFIRM %>" />
            ...
            <table style="width: 50% ; margin-left: auto ; margin-right: auto"  >
                <tr>
                    ...
                    <td style="border: none">
                        <div class="requriedclass">
                            <div>
                                <input name="DonatorPhone"  autocomplete="off" type="text" id="DonatorPhone" maxlength="11" title="<%=CONFIG.DonatorPhoneTitle(session)%>"  onblur="checkAccount(this.id);" >
                            </div>
                        </div>
                    </td>
                     <td style="border: none">
                ...
                <tr>
                    <td style="border: none">
                        <p>فئة الشحن المطلوبه: </p>
                    </td>
                    <td style="border: none">
                        <div class="requriedclass">
                            <div>
                                <select required id="myselection" onchange="changeValue()">
                                 <option value=""> <%=CONFIG.VOUCHER_ONECARD_CHOOSE%> </option>
                                <%
                                    for(OneCardDenominationResponse item: oneCardList){
                                %>
                                <option value="<%=item.getId() %>"> <%=item.getDenominationName() %> </option>
                                <%
                                    }
                                %>
                                </select>
                                <input name="donationValue" id="donationValue" autocomplete="off" type="hidden"/>
                            </div>
                        </div>
                    </td>
                    <td style="border: none">
